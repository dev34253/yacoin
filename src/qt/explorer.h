...
const int nARBITRARY_LARGE_NUMBER_OF_ROWS = 1500, // was 150,
    nONE_ROW = 1;
...
class ExplorerPage : public QDialog
// public QWidget
{
    Q_OBJECT
...
TX_nm               = 4,
    ...
    int nROWS_OF_DISPLAYED_BLOCKS, nROWS_OF_DISPLAYED_TRANSACTIONS;
  ...
  Q_SIGNALS:
    ...
    // This signal is emitted when a mouse button is double-clicked. The item the mouse was
    // double-clicked on is specified by index. The signal is only emitted when the index is valid.
  ...
  private:
    ...
    TransactionExplorerPage *pExplorerTransactionDialog;

    // pointers to models to hold the block data, transaction data, ...
    QStandardItemModel *pQSIMblocks;
    QStandardItemModel *pQSIMtransactions;

    // the actual data that is placed into the model
    QList<QVector<QString>> qlistOfTxRows, qlistOfBlockRows;

    QVector<int> vBlockPeriods;

    int nLastBestHeight;

    std::string sCOIN_SYMBOL;

    bool fCloseButton;
    static // declaration
        bool fDontReenterMe,
        fResizeTxsFields, fResizeBlocksFields;
...
// public QWidget
{
  ...
  public:
    explicit BlockExplorerPage(QDialog *parent = 0);
    ~BlockExplorerPage();
    ...
    QItemSelectionModel *pQISM;

    //    QTextBrowser
    //        *pQTBblock;
    bool fBlkInfoConnected;
  ...
  private:
    ...
    QList<QVector<QString>> qlistOfBlockinfoRows;
...
// public QWidget
{
    Q_OBJECT
  public:
    explicit TransactionExplorerPage(QDialog *parent = 0);
  ...
  private:
    ...
    QList<QVector<QString>> qlistOfTxinfoRows;
