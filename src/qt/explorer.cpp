...
std::string BuildTxDetailsFrom(uint256 &hashOfSelectedTransaction // in
                               ,
                               CTransaction &tx // in
                               ,
...
ExplorerPage::ExplorerPage(const PlatformStyle *platformStyle, QWidget *parent, bool fNoCloseButton)
    : // ExplorerPage::ExplorerPage(QDialog * const parent) :
      ...
      // QWidget(parent)
      ,
      ui(new Ui::ExplorerPage), pExplorerBlockDialog(0), pExplorerTransactionDialog(0)
//, model(0)
{
    ui->setupUi(this);
    ...
    const int nGOOD_impossible_value = -1;
    ...
    bool fOK = connect(pExplorerBlockDialog->pQTVblockinfo,
                       SIGNAL(clicked(QModelIndex)), // SIGNAL(doubleClicked( QModelIndex )),
                       this, SLOT(showBkInfoDetails(QModelIndex)));
    if (fOK)
        pExplorerBlockDialog->fBlkInfoConnected = true;

    fOK = connect(pExplorerTransactionDialog->pQTVtxinfo,
                  SIGNAL(clicked(QModelIndex)), // SIGNAL(doubleClicked( QModelIndex )),
    ...
    for (int column = 0; column < BLOCK_VIEW_SIZE; ++column) {
        pQSIMblocks->setHorizontalHeaderItem(
            column, new QStandardItem(QString(ppcArrayOfBlockHeaders[column])));
    ...
    for (int column = 0; column < TX_VIEW_SIZE; ++column) {
        QString QS = QString(ppcArrayOfTransactionHeaders[column]);

        QStandardItem *pQSI = new QStandardItem(QS);
...
void ExplorerPage::showBkDetails(QModelIndex QMI) {
    ...
    int nColumn = QMI.column(), nRow = QMI.row();

    if ((nRow < qlistOfBlockRows.size()) && // only existing rows
        (HEIGHT == nColumn)                 // only 'height' column
    ) {                                     // clicked somewhere in the height column
        bool fOK;
        QString QSblockNumber = pQSIMblocks->item(nRow, HEIGHT)->text();
        int nHeight = QSblockNumber.toInt(&fOK);

        if (!fBlockEditConnected) {
            pop_a_message_box("Something's wrong in slotland!");
        }
        if (fOK) { // copy nHeight to block # input line edit field
...
void ExplorerPage::showBlockLineDetails() {
    bool fOK;
    int nNewBlockNumber = pBlockNumberLineEdit->text().toInt(&fOK);

    if (fOK) {
        if ((0 <= nNewBlockNumber) && (nNewBlockNumber <= chainActive.Height())) {
...
void ExplorerPage::showTxIdLineDetails() {
    QString QShash = pTxIDLineEdit->text();

    std::string sTemp = QShash.toStdString();

    uint256 hashOfSelectedTransaction(sTemp);
    ...
    if (GetTransaction(hashOfSelectedTransaction, tx, hashBlock)) {
        sTemp = BuildTxDetailsFrom(hashOfSelectedTransaction, tx, hashBlock,
...
void ExplorerPage::showBlockHashLineDetails() {
    QString QShash = pBlockHashLineEdit->text();

    std::string sTemp = QShash.toStdString();

    uint256 hashOfSelectedItem(sTemp);

    CBlockIndex *pblockindex = mapBlockIndex[hashOfSelectedItem];
    ...
    if (pblockindex) {
        ...
        std::string sX =
            BuildBlockinfoDetailsFrom(block, pblockindex, pExplorerBlockDialog->pQSIMblockinfo,
...
//_____________________________________________________________________________
// SLOT receiver object for clicking or double clicking on a Tx in the table
// view
//_____________________________________________________________________________
...
void ExplorerPage::showTxDetails(QModelIndex QMI) {
    ...
    if ((nRow < qlistOfTxRows.size()) &&
        (TX_ID == nColumn)) { // clicked somewhere in the ID column, where there is data
        QString QShash = pQSIMtransactions->item(nRow, TX_ID)->text();
...
void ExplorerPage::showTxInfoDetails(QModelIndex QMI) {
    int row = QMI.row(), column = QMI.column();

    QString QSitem = pExplorerTransactionDialog->pQSIMtxinfo
                         ->item(row, pExplorerTransactionDialog->TRANSACTION_INFO_ITEM)
                         ->text(),
            QSvalue = pExplorerTransactionDialog->pQSIMtxinfo
                          ->item(row, pExplorerTransactionDialog->TRANSACTION_INFO_VALUE)
                          ->text();
    // what was clicked?
    if (("Tx ID" == QSitem.toStdString()) || ("Coinbase hash" == QSitem.toStdString())) {
        // pop_a_message_box( "A tx hash " + QSitem.toStdString() );
    }
    if (("Tx's block hash" == QSitem.toStdString())) {
        // pop_a_message_box( "A block hash " + QSitem.toStdString() );
        uint256 hashOfSelectedItem(QSvalue.toStdString());
    ...
    pExplorerTransactionDialog->pQTVtxinfo->selectionModel()->select(QMI,
...
void ExplorerPage::showBkInfoDetails(QModelIndex QMI) {
    ...
    QString QSitem = pExplorerBlockDialog->pQSIMblockinfo
                         ->item(row, pExplorerBlockDialog->BLOCK_INFO_ITEM)
                         ->text(),
            QSvalue = pExplorerBlockDialog->pQSIMblockinfo
                          ->item(row, pExplorerBlockDialog->BLOCK_INFO_VALUE)
                          ->text();

    QString QShash = QSvalue.toStdString().c_str();

    std::string sTemp = QShash.toStdString();

    uint256 hashOfSelectedValue(sTemp);

    CTransaction tx;

    uint256 hashBlock = 0;

    if (("previous block" == QSitem.toStdString()) || ("hashPrevBlock" == QSitem.toStdString()) ||
        ("next block" == QSitem.toStdString())) {
        QShash = QSvalue.toStdString().c_str();

        CBlockIndex *pblockindex = mapBlockIndex[hashOfSelectedValue];
    ...
    const int nBLOCK_HASH_ROW = 0;

    QString QSblockhash = pExplorerBlockDialog->pQSIMblockinfo
                              ->item(nBLOCK_HASH_ROW, pExplorerBlockDialog->BLOCK_INFO_VALUE)
                              ->text();
    uint256 hashOfBlock(QSblockhash.toStdString());
    ...
    int nThisVTxSize = (int)thisblock.vtx.size(),
        nThisVMerkleSize = (int)thisblock.vMerkleTree.size();

    QString QStxhash = "";

    if (1 == nThisVTxSize) {
        if (("merkle root hash" == QSitem.toStdString()) ||
    ...
    {
        if ((strprintf("vMerkleTree[ %d ]", i).c_str() == QSitem.toStdString())) {
            QStxhash = QSvalue.toStdString().c_str();
        }
    }
    if ("" != QStxhash.toStdString()) {
        std::string sTemp = QStxhash.toStdString();
        ...
        if (GetTransaction(hashOfSelectedItem, tx, hashBlock)) {
            std::string sTemp = BuildTxDetailsFrom(hashOfSelectedItem, tx, hashBlock,
                                                   pExplorerTransactionDialog->pQSIMtxinfo,
                                                   pExplorerTransactionDialog->pQTVtxinfo);
            // it seems one must move the focus in Qt or this selection
            // lingers!?
            pExplorerTransactionDialog->show();
            // pExplorerTransactionDialog->raise( );
            pExplorerTransactionDialog->setVisible(true);
...
std::string BuildTxDetailsFrom(uint256 &hashOfSelectedTransaction, CTransaction &tx,
                               uint256 &hashBlock, QStandardItemModel *pQSIMtxinfo,
                               QTableView *pQTVtxinfo) {
    int nRowCount = 0, TX_INFO_ITEM = TransactionExplorerPage::TRANSACTION_INFO_ITEM,
        TX_INFO_VALUE = TransactionExplorerPage::TRANSACTION_INFO_VALUE;

    int nNumberOfRows = pQSIMtxinfo->rowCount();

    if (nNumberOfRows > 0) {
        pQSIMtxinfo->setRowCount(nARBITRARY_LARGE_NUMBER_OF_ROWS);
    }

    std::string sStandardItemModelElement, sTemp = "function BuildTxDetailsFrom()\n";

    sTemp = strprintf("function BuildTxinfoDetailsFrom()\n"
                      "%s",
                      //"<br />"
                      "<h3 >"
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s", hashOfSelectedTransaction.GetHex().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
    //_________________________________________________________________________
    if (0 == hashBlock) {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            if (pindex->IsInMainChain()) {
                sStandardItemModelElement =
                    strprintf("%d", 1 + chainActive.Height() - pindex->nHeight);
                sTemp += sStandardItemModelElement + "<br />\n";
                pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                                     sStandardItemModelElement.c_str());
                ++nRowCount;
                ...
                sStandardItemModelElement = strprintf(
                    //" %lld"
                    " %" PRI64d "", (boost::int64_t)pindex->nTime);
                sTemp += sStandardItemModelElement + "<br />\n";
                pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                                     sStandardItemModelElement.c_str());
                ++nRowCount;
    ...
    std::string sTime =
        QDateTime::fromTime_t(tx.nTime).toString("hh:mm:ss MM/dd/yyyy").toStdString();

    sStandardItemModelElement =
        tx.IsCoinBase() ? "Coinbase" : (tx.IsCoinStake() ? "Coinstake" : "CTransaction");
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %ld (local %s)", tx.nTime, sTime.c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %d", tx.nVersion);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    int nVIsize = int(tx.vin.size());

    sStandardItemModelElement = strprintf(" %" PRIszu "", tx.vin.size());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    int nVOsize = int(tx.vout.size());
    sStandardItemModelElement = strprintf(" %" PRIszu "", tx.vout.size());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_ITEM, QModelIndex()),
                         sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %d", tx.nLockTime);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMtxinfo->setData(pQSIMtxinfo->index(nRowCount, TX_INFO_VALUE, QModelIndex()),
                         sStandardItemModelElement.c_str());
    ++nRowCount;
...
ExplorerPage::~ExplorerPage() { delete ui; }
...
void ExplorerPage::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header) {
    ...
    // if(pclientModel)
    {
        QModelIndex // an index into the block Model of the explorer page
            BlockTableIndex;
        static const int nNinetySeconds = 90, // arbitrary moment to get YAC price
        ...
        bool fMostlyUpToDate = false;

        QDateTime Qnow = QDateTime::currentDateTime(),
                  QdateOfBestBlock = QDateTime::fromTime_t(pblockindex->GetBlockTime());
        int nDeltaSecondsAgeOfBlock =
            QdateOfBestBlock.secsTo(Qnow); // can be + or - some seconds from now

        if (nDeltaSecondsAgeOfBlock < (int)nTwelveHoursInSeconds) // OK to update explorer window
        {
            // now at 1 minute/ block, 12 hours is ~720 blocks arbitrary point
            // to start the display of data
            QVector<QString> vStringBlockDataRow(BLOCK_VIEW_SIZE); // our model behind the model

            CBlock block;
            const Consensus::Params &consensusParams = Params().GetConsensus();
            if (ReadBlockFromDisk(block, pblockindex,
                                  consensusParams)) // true means read Tx's, false means don't
            {
                ...
                //_____________________________________________________________
                //_____________________________________________________________
                std::string strTemporary = strprintf("%d", pblockindex->nHeight);
                ...
                //_____________________________________________________________
                strTemporary =
                    strprintf("%.3f %s", pblockindex->nMint / 1000000.0, sCOIN_SYMBOL.c_str());
                vStringBlockDataRow[TOTAL_SENT] = strTemporary.c_str();

                //_____________________________________________________________
                int nS = (int)::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION);
                ...
                // so we have a new vector of strings, vStringBlockDataRow,
                // perfect for out qlistOfBlockRows
                //_________________________________________________________________

                // massaging our Qlist
                bool fMatched = false, // represents the answer to the question
                                       // do we already have this block in our list,
                                       // which is in our model, which is in our view
                                       // (whew!). We begin with a presumed false,
                                       // since or model is empty.
                    fAddedRow = false;
                ...
                {      // so let's delete if neccessary
                    ...
                    for (int row = 0; row < nNumberOfRows; ++row) // what do we have so far?
                    {
                        ...
                        if (!fBool)                 // something is amiss??
                            continue;               // swallow hard and check the next row !
                                                    // :)
                        if (nHeight == nTestHeight) // we matched height, let's
                                                    // replace with the
                        {                           // latest Age
                            fMatched = true;
                            break;
                    ...
                    {
                        if (nNumberOfRows >= nROWS_OF_DISPLAYED_BLOCKS) // model is full, so
                                                                        // delete first row in
                                                                        // our Qlist
                    ...
                    // else we matched so don't add the same block, i.e. do
                ...
                {                             // by putting the model into the view
                    // now, qlistOfBlockRows.size() is complete for display, so
                    // let's put qlistOfBlockRows into model, then the model
                    // into the view

                    int nSize = qlistOfBlockRows.size(); // known to be <= nROWS_OF_DISPLAYED_BLOCKS

                    for (int row = 0; row < nSize; ++row) {
                        for (int column = 0; column < BLOCK_VIEW_SIZE; ++column) {
                            BlockTableIndex = pQSIMblocks->index(row, column, QModelIndex());
                            if (AGE == column) // recalculate the age of the
                                               // older blocks
                            {
                                bool fOK;
                                int nBn = qlistOfBlockRows[row][HEIGHT].toInt(&fOK);
                                if (fOK) {
                                    ...
                                    int64_t nTimeOfBlock = pblockindex->GetBlockTime(),
                                            nQTimeNow = Qnow.toMSecsSinceEpoch() / 1000;

                                    int nBlockAge = (int)(nQTimeNow - nTimeOfBlock);

                                    // let's save these times for averageing
                                    // purposes
                                    vBlockPeriods[row] = nBlockAge;

                                    if (nBlockAge > 0) {
                                        const int nONE_HOUR_in_seconds = 60 * 60;
                                    ...
                                    pQSIMblocks->setData(BlockTableIndex, strTemporary.c_str());
                    ...
                    if (!fResizeBlocksFields) {
                        ...
                        if (!pExplorerTransactionDialog->fTxInfoConnected) {
                            pop_a_message_box("connection failed for "
                                              "Transaction View signal");
                        }
                        for (int column = 0; column < BLOCK_VIEW_SIZE; ++column)
                            pQTVblocks->resizeColumnToContents(column);
                    } else {
                    ...
                    // lets's calculate our running average block period here
                    // since the times are all "fresh"
                    int nCounter = 0, nTotalAges = 0;
                    ...
                    {
                        if (nTotalAges > 0) {
                            int nAverageBlockPeriod =
                                nTotalAges / nCounter; // since it's seconds, we
                                                       // are happy with whole seconds
                            // const int
                            //    nONE_HOUR_in_seconds = 60 * 60;

                            if (nAverageBlockPeriod >= nSECONDsPerHOUR)
                                pQLaverage->setText(QDateTime::fromTime_t(nAverageBlockPeriod)
                                                        .toUTC()
                                                        .toString("hh:mm:ss"));
                            else
                                pQLaverage->setText(QDateTime::fromTime_t(nAverageBlockPeriod)
        ...
        //_____________________________________________________________________________
        //_____________________________________________________________________________
        // now check transactions similarly to blocks, but get the Tx hash from
        // elsewhere
        QModelIndex TransactionTableIndex;

        // static uint256
        uint256 lastHashKnown = 0;

        bool fMatched = true;

        uint256 latestHashSeen = lastTxHash.retrieveLastHash(); // our hook from main.cpp's
                                                                // CTxMemPool::accept()

        if ((0 != latestHashSeen) && (lastHashKnown != latestHashSeen)) // work to do
        {
            fMatched = false;
            lastHashKnown = latestHashSeen;

            CTransaction CrefToTx;
            ...
            if (mempool.exists(latestHashSeen)) {
                ...
                //_____________________________________________________________
                strTemporary =
                    strprintf("%.3f %s", CrefToTx.GetValueOut() / 1000000.0, sCOIN_SYMBOL.c_str());
                vStringTransactionDataRow[TX_AMOUNT] = strTemporary.c_str();

                //_____________________________________________________________
                int64_t nTimeOfTx = CrefToTx.nTime;

                strTemporary =
                    QDateTime::fromTime_t(nTimeOfTx).toString("hh:mm:ss MM/dd/yyyy").toStdString();
                vStringTransactionDataRow[TX_TIME] = strTemporary.c_str();

                //_____________________________________________________________
                int64_t nQTimeNow = Qnow.toMSecsSinceEpoch() / 1000; // this is now in seconds
                int nTxAge = (int)(nQTimeNow - nTimeOfTx);

                if (nTxAge > 0)
                    strTemporary =
                        QDateTime::fromTime_t(nTxAge).toUTC().toString("hh:mm:ss").toStdString();
                else
                ...
                {
                    int nNumberOfRows = qlistOfTxRows.size();

                    // if the tx id doesn't match any tx we are already
                    // displaying, then add, i.e. prepend()
                    for (int row = 0; row < nNumberOfRows; ++row) {
                        uint256 hashTest(pQSIMtransactions->item(row, TX_ID)->text().toStdString());
                    ...
                    if (!fMatched) {
                        if (nNumberOfRows >= nROWS_OF_DISPLAYED_TRANSACTIONS)
                            qlistOfTxRows.removeLast(); // model is full, so delete last
                                                        // row in our Qlist
                        qlistOfTxRows.prepend(vStringTransactionDataRow); // so we have added a
                                                                          // row
                    }
                    // else  //we matched so don't add the same Transaction,
                    // i.e. do nothing
                }

                if (!fMatched) { // this puts the model into the view
                    std::string sHashToAvoid = CrefToTx.GetHash().ToString();
                    int nNumberOfRows = qlistOfTxRows.size();
                    // really should recalculate times of all but the first row
                    for (int row = 0; row < nNumberOfRows; ++row) {
                        for (int column = 0; column < TX_VIEW_SIZE; ++column) {
                            TransactionTableIndex =
                                pQSIMtransactions->index(row, column, QModelIndex());
                            if (TX_AGE == column) // recalculate the age displayed of the
                                                  // older transactions
                            {
                                bool fOK;

                                std::string sV = qlistOfTxRows[row][TX_ID].toStdString();

                                if (sV != sHashToAvoid) // we need to update the age
                                {                       // first get the Tx from the hash
                                    CTransaction CrefToAnOldTx;
                                    uint256 hashToMatch,
                                    ...
                                    bool fFoundTx =
                                        GetTransaction(hashToMatch, CrefToAnOldTx, hashOfBlock);
                                    if (fFoundTx) {
                                        int64_t nTimeOfTx = CrefToAnOldTx.nTime;
                                        int nTxAge = (int)(nQTimeNow - nTimeOfTx);
                                        std::string strTemporary;

                                        if (nTxAge > 0)
                                            strTemporary = QDateTime::fromTime_t(nTxAge)
                                                               .toUTC()
                                                               .toString("hh:mm:ss")
                                                               .toStdString();
                                        else
                                            strTemporary = strprintf("%d sec", nTxAge);
                                        pQSIMtransactions->setData(TransactionTableIndex,
                                                                   strTemporary.c_str());
                                    } else // couldn't find the hash??
                                    {
                                        pQSIMtransactions->setData(
                                            TransactionTableIndex,
                                            qlistOfTxRows[row][column].toStdString().c_str());
                                    }
                                } else // this is the new one, so it's done
                                       // already
                                    pQSIMtransactions->setData(
                                        TransactionTableIndex,
                                        qlistOfTxRows[row][column].toStdString().c_str());
            ...
            }  // else  // the hash doesn't exist (anymore?) so no  transaction
               // info
        ...
        {      // but let's freshen all the Ages, so this will happen ~1 minute since
            ...
            if (!qlistOfTxRows.isEmpty()) // there are rows whose times need updating
            {
                ...
                for (int row = 0; row < nNumberOfRows; ++row) {
                    ...
                    TransactionTableIndex = pQSIMtransactions->index(row, column, QModelIndex());
                    std::string sTxID = qlistOfTxRows[row][TX_ID].toStdString();
                    ...
                    bool fFoundTx = GetTransaction(hashToMatch, CrefToAnOldTx, hashOfBlock);
                    if (fFoundTx) {
                        int64_t nTimeOfTx = CrefToAnOldTx.nTime;
                        int nTxAge = (int)(nQTimeNow - nTimeOfTx);
                        std::string strTemporary;
                        ...
                        if (nTxAge > 0)
                            ...
                            strTemporary = strprintf("%d sec", nTxAge);
                        pQSIMtransactions->setData(TransactionTableIndex, strTemporary.c_str());
                    }
                }
                pQTVtransactions->setModel(pQSIMtransactions);
                pQTVtransactions->setEditTriggers(QAbstractItemView::NoEditTriggers);
...
//, model(0)
{
    ...
    char *pcArrayOfBlockinfoHeaders[BLOCK_INFO_VIEW_SIZE];

    pcArrayOfBlockinfoHeaders[BLOCK_INFO_ITEM] = "Name";
    pcArrayOfBlockinfoHeaders[BLOCK_INFO_VALUE] = "Value";

    pQSIMblockinfo =
        new QStandardItemModel(nARBITRARY_LARGE_NUMBER_OF_ROWS, BLOCK_INFO_VIEW_SIZE, this);

    for (int column = 0; column < BLOCK_INFO_VIEW_SIZE; ++column) {
        pQSIMblockinfo->setHorizontalHeaderItem(
            column, new QStandardItem(QString(pcArrayOfBlockinfoHeaders[column])));
...
//_____________________________________________________________________________
std::string BuildBlockinfoDetailsFrom(CBlock &block, CBlockIndex *pblockindex,
                                      QStandardItemModel *pQSIMblockinfo,
                                      QTableView *pQTVblockinfo) {
    int nRowCount = 0, nNumberOfRows = pQSIMblockinfo->rowCount();

    if (nNumberOfRows > 0) {
        pQSIMblockinfo->setRowCount(nARBITRARY_LARGE_NUMBER_OF_ROWS);
    }

    int BLOCK_INFO_ITEM = BlockExplorerPage::BLOCK_INFO_ITEM,
        BLOCK_INFO_VALUE = BlockExplorerPage::BLOCK_INFO_VALUE;

    std::string sTime =
        QDateTime::fromTime_t(block.nTime).toString("hh:mm:ss MM/dd/yyyy").toStdString();

    CBigNum bnTarget;

    bnTarget.SetCompact(block.nBits);

    uint256 hTarget = bnTarget.getuint256();

    std::string sTarget = hTarget.GetHex();

    QModelIndex BlockinfoTableModelIndex;

    std::string sStandardItemModelElement,
        sTemp = strprintf("function BuildBlockinfoDetailsFrom()\n"
                          "%s",
                          //"<br />"
                          "<h3 >"
    ...
    sTemp += "<b >" + sStandardItemModelElement + "</b >";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
    ...
    if (pblockindex->pprev) {
        ...
        pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                                sStandardItemModelElement.c_str());
        sStandardItemModelElement =
            strprintf("%s", pblockindex->pprev->GetBlockHash().ToString().c_str());
        sTemp += "<font color = 'blue' ><a href = '' >" + sStandardItemModelElement +
                 "</a ></font ><br />\n";
        pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                                sStandardItemModelElement.c_str());
        ++nRowCount;
    }
    //_________________________________________________________________________
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %u ", pblockindex->nFile);
    sTemp += sStandardItemModelElement + ")<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" (%-6d)", pblockindex->nDataPos);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %d ", pblockindex->nHeight);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s, ", FormatMoney(pblockindex->nMint).c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s ", FormatMoney(pblockindex->nMoneySupply).c_str());
    sTemp += sStandardItemModelElement + "<br />";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    sStandardItemModelElement =
        strprintf(" (%s)", pblockindex->GeneratedStakeModifier() ? "MOD" : "-");
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" (%d)", pblockindex->GetStakeEntropyBit());
    sTemp += sStandardItemModelElement;
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement =
        strprintf(" (Po%s)", pblockindex->IsProofOfStake() ? "S" : "W" // i.e. PoS or PoW
        );
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %016" PRI64x " ", pblockindex->nStakeModifier);
    sTemp += sStandardItemModelElement + "<br />";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %08x ", pblockindex->nStakeModifierChecksum);
    sTemp += sStandardItemModelElement + "<br />";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    if (0 != pblockindex->hashProofOfStake) {
        std::string sHPOS1 = "hashProofOfStake = ", sHPOS2 = "", sHPOS3 = "\n", sHPOS = "";
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" (%s) ", pblockindex->prevoutStake.ToString().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %d ", pblockindex->nStakeTime);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s ", pblockindex->hashMerkleRoot.ToString().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s"
                                          "",
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s", block.GetHash().ToString().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %d", block.nVersion);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s", block.hashPrevBlock.ToString().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %s", block.hashMerkleRoot.ToString().c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %ld (local %s)", block.nTime, sTime.c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    const int nArbitaryHexDigitsToShow = 16;

    sStandardItemModelElement = "nBits";
    sTemp += "<b >" + sStandardItemModelElement + "</b >";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %08x (target %s...)", block.nBits,
                                          sTarget.substr(0, nArbitaryHexDigitsToShow).c_str());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %u(dec) %08x(hex)", block.nNonce, block.nNonce);
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                            sStandardItemModelElement.c_str());
    sStandardItemModelElement = strprintf(" %" PRIszu " ", block.vtx.size());
    sTemp += sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ++nRowCount;
    ...
    // sStandardItemModelElement = strprintf(
    //                    " %s"
    //                    , HexStr(block.vchBlockSig.begin(),
    //                    block.vchBlockSig.end()).c_str()
    //                                     );
    ...
    //_________________________________________________________________________
    int nSize = block.vMerkleTree.size();
    ...
    if (0 == nSize) {
        ...
        sStandardItemModelElement = strprintf("  %s ", block.vMerkleTree[0].ToString().c_str());
    }
    sTemp += "&nbsp;&nbsp;" + sStandardItemModelElement + "<br />\n";
    pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                            sStandardItemModelElement.c_str());
    ...
    for (int i = 1; i < nSize; ++i) {
        ...
        pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_ITEM, QModelIndex()),
                                sStandardItemModelElement.c_str());
        sStandardItemModelElement = strprintf("  %s ", block.vMerkleTree[i].ToString().c_str());
        sTemp += "&nbsp;&nbsp;" + sStandardItemModelElement + "<br />\n";
        pQSIMblockinfo->setData(pQSIMblockinfo->index(nRowCount, BLOCK_INFO_VALUE, QModelIndex()),
                                sStandardItemModelElement.c_str());
        ++nRowCount;
...
void BlockExplorerPage::fillBlockInfoPage(int currentHeight) {
    ...
    {
        std::string sX =
            BuildBlockinfoDetailsFrom(block, pblockindex, pQSIMblockinfo, pQTVblockinfo);
...
void BlockExplorerPage::on_closeButton_clicked() { close(); }
...
//, model(0)
{
    ...
    char *pcArrayOfTxinfoHeaders[TRANSACTION_INFO_VIEW_SIZE];

    pcArrayOfTxinfoHeaders[TRANSACTION_INFO_ITEM] = "Name";
    pcArrayOfTxinfoHeaders[TRANSACTION_INFO_VALUE] = "Value";

    pQSIMtxinfo =
        new QStandardItemModel(nARBITRARY_LARGE_NUMBER_OF_ROWS, TRANSACTION_INFO_VIEW_SIZE, this);
    for (int column = 0; column < TRANSACTION_INFO_VIEW_SIZE; ++column) {
        pQSIMtxinfo->setHorizontalHeaderItem(
            column, new QStandardItem(QString(pcArrayOfTxinfoHeaders[column])));
