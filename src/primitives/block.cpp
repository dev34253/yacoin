        ...
        bool fReadTransactions, bool fCheckHeader)
{
    ...
    if (fBlockHashIndex && !pblocktree->ReadBlockHash(nFile, nBlockPos, blockHash))
    {
...
//                                          << ", " << i << ", " << hash.GetHex() << ", " << j << ", " << "true" << ", " << tokenAmount * -1 << std::endl;
    ...
    if (pindex->pprev)
    {
        ...
        if (fBlockHashIndex && !pblocktree->WriteBlockHash(blockindexPrev))
        {
...
//_____________________
    ...
    if (fBlockHashIndex && !pblocktree->WriteBlockHash(CDiskBlockIndex(pindex)))
    {
