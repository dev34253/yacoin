...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE)
    {
        ...
        LogPrintf(
            ...
            "size %" PRIszu
            " which exceeds maximum size %i thus can never be redeemed. Do not "
            "use address %s.\n",
    ...
    // restored from backup or the user making copies of wallet.dat.
    {
        ...
        for(const CTxIn& txin : tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                {
                    LogPrintf("WalletUpdateSpent found spent coin %syac %s\n",
...
{
    return CWalletDB(*pwallet->dbw).WriteTx(*this);
}
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    while (fRepeat)
    {
        ...
        for(PAIRTYPE(const uint256, CWalletTx)& item : mapWallet)
        {
            ...
            bool fUpdated = false;
            bool fFound = view.HaveInputs(wtx);
            if (fFound || wtx.GetDepthInMainChain() > 0)
            {
                ...
                for (unsigned int i = 0; i < wtx.vout.size(); i++)
                {
                    if (wtx.IsSpent(i))
                        continue;
                    const Coin& existingCoin = view.AccessCoin(COutPoint(hashTx, i));
                    if (existingCoin.IsSpent() && IsMine(wtx.vout[i]))
                    {
                        wtx.MarkSpent(i);
                        fUpdated = true;
                        fMissing = true;
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %syac %s\n", FormatMoney(wtx.GetCredit(MINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
        ...
        std::set<CInputCoin> &setCoinsRet, CAmount &nValueRet) const
{
        ...
        ((nBest != nTargetValue && nBest < nTargetValue + CENT) || coinLowestLarger->txout.nValue <= nBest))
    {
        ...
        if (fDebug && gArgs.GetBoolArg("-printpriority"))
        {
            ...
            for (unsigned int i = 0; i < vValue.size(); i++)
                if (vfBest[i])
                    LogPrintf("%s ", FormatMoney(vValue[i].txout.nValue));
            LogPrintf("total %s\n", FormatMoney(nBest));
        }
    }
...
//  std::vector<CInputCoin> vValue;
  ...
  if (coinLowestLarger && coinLowestLargerAmount &&
    ...
    if (fDebug && gArgs.GetBoolArg("-printpriority")) {
      //// debug print
      LogPrintf("SelectTokens() best subset: ");
      for (unsigned int i = 0; i < vValue.size(); i++)
        if (vfBest[i])
            LogPrintf("%s : %s ", strTokenName,
                 FormatMoney(vValue[i].second));
...
{
    {
...

        {
            ...
            CWalletDB* pwalletdb = fFileBacked ? new CWalletDB(*dbw, "r") : NULL;
...
CWallet* CWallet::CreateWalletFromFile(const std::string walletFile)
{
    ...
    if (fFirstRun)
    {
        ...
        if (walletInstance->GetKeyFromPool(newDefaultKey, false)) {
            ...
            if (!walletInstance->SetAddressBookName(walletInstance->vchDefaultKey.GetID(), "")) {
...
bool CWallet::InitLoadWallet()
{
    ...
    std::string walletFile = gArgs.GetArg("-wallet", "wallet.dat");
...
{
    {
        ...
        if (block.IsProofOfStake() && mapWallet.count(block.vtx[1].GetHash()))
        {
            ...
            LogPrintf("    PoS: %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit(MINE_ALL));
        }
        else if (mapWallet.count(block.vtx[0].GetHash()))
        {
            CWalletTx& wtx = mapWallet[block.vtx[0].GetHash()];
            LogPrintf("    PoW:  %d  %d  %" PRId64 "", wtx.GetDepthInMainChain(), wtx.GetBlocksToMaturity(), wtx.GetCredit(MINE_ALL));
        }
    }
    LogPrintf("\n");
}
...
{
    {
        ...
        LogPrintf("CWallet::NewKeyPool wrote %" PRIu64 " new keys\n", nKeys);
    }
    return true;
}
...
{
    {
        ...
        while (setKeyPool.size() < (nTargetSize + 1))
        {
            ...
            LogPrintf("keypool added key %" PRIu64 ", size=%" PRIszu "\n", nEnd, setKeyPool.size());
        }
    }
    return true;
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    for(CWalletTx* pcoin : vCoins)
    {
        ...
        if (view.HaveInputs(*pcoin))
            continue;
        const uint256& hashTx = pcoin->GetHash();
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            const Coin& existingCoin = pcoinsTip->AccessCoin(COutPoint(hashTx, n));
            if (IsMine(pcoin->vout[n]) && pcoin->IsSpent(n) && !existingCoin.IsSpent())
            {
              LogPrintf("FixSpentCoins found lost coin %sppc %s[%d], %s\n",
                        FormatMoney(pcoin->vout[n].nValue),
                        pcoin->GetHash().ToString(), n,
            ...
            else if (IsMine(pcoin->vout[n]) && !pcoin->IsSpent(n) && existingCoin.IsSpent()) {
        ...
        {
            LogPrintf("FixSpentCoins %s tx %s\n", fCheckOnly ? "found" : "removed", pcoin->GetHash().ToString());
            if (!fCheckOnly)
            {
...
void CWallet::GetKeyBirthTimes(std::map<CTxDestination, int64_t> &mapKeyBirth) const
{
    ...
    for (std::map<CTxDestination, CKeyMetadata>::const_iterator it = mapKeyMetadata.begin();
