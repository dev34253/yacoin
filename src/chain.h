...
public:
    ...
    //! (memory only) Total amount of work in the chain up to and including this block
...
/** Used to marshal pointers into hashes for old db storage (need for upgrading from old versions to v1.5.0) */
class CDiskBlockIndexOld : public CBlockIndex
{
public:
    uint256 hashPrev;
    uint256 hashNext;
    ...
    explicit CDiskBlockIndexOld(CBlockIndex* pindex) : CBlockIndex(*pindex)
    {
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        {
            const_cast<CDiskBlockIndexOld*>(this)->prevoutStake.SetNull();
            const_cast<CDiskBlockIndexOld*>(this)->nStakeTime = 0;
            const_cast<CDiskBlockIndexOld*>(this)->hashProofOfStake = 0;
        }
    ...
    uint256 GetBlockHash() const
    {
        ...
        const_cast<CDiskBlockIndexOld*>(this)->blockHash = block.GetHash();
    ...
    std::string ToString() const
    {
        std::string str = "CDiskBlockIndexOld(";
        str += CBlockIndex::ToString();
