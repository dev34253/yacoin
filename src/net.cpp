...
std::vector<AddedNodeInfo> CConnman::GetAddedNodeInfo()
{
    ...
    for (const std::string& strAddNode : lAddresses) {
        CService service(LookupNumeric(strAddNode.c_str(), GetDefaultPort()));
        if (service.IsValid()) {
    ...
    while (true)
    {
        ...
        for (const AddedNodeInfo& info : vInfo) {
            if (!info.fConnected) {
                ...
                tried = true;
                CService service(LookupNumeric(info.strAddedNode.c_str(), GetDefaultPort()));
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. yacoind is probably already running."), addrBind.ToString());
        else
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    CMessageHeader hdr(msg.command.c_str(), nMessageSize);
