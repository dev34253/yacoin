...
def main():
    ...
    tmpdir = "{}/test_runner_{}".format(args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, runs_ci, use_term_control):
    ...
    try:
        if subprocess.check_output(["pidof", "yacoind"]) not in [b'']:
            print("%sWARNING!%s There is already a yacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
